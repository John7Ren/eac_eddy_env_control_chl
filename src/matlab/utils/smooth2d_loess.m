...
%Inoput data as non function
% data=data; %im_resize(data,1440,960);
% xgrid= lonbins;
% ygrid = latbins;
% span_x= 6;
% span_y = 6;
...
for j=sy%    % only run for sy =1:350 when making anomalies
    

%%   progress_bar(j,length(sy))
   % find how far each data point is from the one we care about (in 1/10
   % units?)
   dy = mygrid - ygrid_est(j);
   dy2 = dy.*dy;
   jgood = find((dy2+nan_data_loc) < 1); % identifies all points within yspan of j that arent land
%%
   if length(jgood) >= npt_threshold,
      ...
      data2 = data(jgood);
%%
      for i=sx% [440-span_x*4:440,441+span_x*8:441+span_x*12]  Change to only redo the padded area 
% If grid isnt the same OR it is and we want to smooth over missing data OR
% data point simply sint nan
...
%%
	    ...
	    if length(i1) >= npt_threshold,
	       ...
	       dist = dx.*dx + dy2_sel;  %distance formula but doesnt need sqrt since just seeing if less than 1
	       igood = find(dist < 1);
	       ngood = length(igood);
...
%---
% Changed 11/15/2011
warning('off','MATLAB:nearlySingularMatrix') % Turn back on for new vairrables but I think this is just a column in the padding
